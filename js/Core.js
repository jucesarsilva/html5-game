...
class Core {
  ...
  startThemeMusic() {
    this.#themeMusic = new Audio("sounds/starcraft.mp3");
  ...
  startCollections() {
    ...
    for (var k = 0; k < this.#qtdMaxEnemies; k++) {
      this.#arrayObjEnemies[k].randomPosition();
    }
  }
    ...
    if (this.#keyboard.getKeySpace()) {
      if (!this.#player.isRespaw()) {
        if (this.#player.fire) {
          ...
          index = this.#arrayObjPlayerShot.length;
          this.#arrayObjPlayerShot.push(shotPlayer);

          this.#arrayObjPlayerShot[index].setX(this.#player.getX() + (this.#player.getWidth() / 2));
          this.#arrayObjPlayerShot[index].setY(this.#player.getY() + (this.#player.getHeight() / 2.5));

          this.#arrayObjPlayerShot[index].setSpeed((this.#player.getSpeed() / 4) + 7);
  ...
  updateEnemiesPositions() {
    for (let i = 0; i < this.#qtdMaxEnemies; i++) {
      ...
      if (this.#arrayObjEnemies[i].getX() < -100) {
        this.#arrayObjEnemies[i].randomPosition();
      }
  ...
  checkAllColision() {
    if (!this.#player.isRespaw()) {
      for (let v = 0; v < this.#arrayObjEnemies.length; v++) {
        if (isCollision(this.#player, this.#arrayObjEnemies[v])) {
          this.#arrayExplosionEnemies[v].play(this.#arrayObjEnemies[v].getX(), this.#arrayObjEnemies[v].getY());
          this.#arrayObjEnemies[v].randomPosition();
          this.#deads++;
          this.#explosionEnemiesObj[v].play();
          this.#player.play();
        } else if (isCollision(this.#player, this.#arrayObjShotEnemies[v])) {
    ...
    if (this.#arrayObjPlayerShot.length > 0) {
      for (let k = 0; k < this.#arrayObjPlayerShot.length; k++) {
        for (let e = 0; e < this.#arrayObjEnemies.length; e++) {
          if (isCollision(this.#arrayObjPlayerShot[k], this.#arrayObjEnemies[e])) {
          ...
          } else if (isCollision(this.#arrayObjPlayerShot[k], this.#arrayObjShotEnemies[e])) {
            if (!this.#player.isRespaw()) {
